...
Считается по распискам: тело расписки и есть отпечаток той версии, которую
ревьюер видел, поэтому пересчитывать дифф не нужно. Git всё же зовётся —
ровно один раз, чтобы узнать текущую ветку и выбрать нужную папку расписок.
