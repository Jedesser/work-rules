...
def is_opt(tok: str, canonical: str, min_len: int = 3) -> bool:
    """Тот ли это длинный флаг — с учётом сокращений, которые понимает git.

    git принимает однозначное сокращение: `--al` — это `--all`, `--amen` —
    `--amend`, `--har` — `--hard`. Сравнение дословной строкой пропускает
    ровно ту же команду, которую запрещает, а сокращать флаги никто не
    мешает. `min_len` считает и два дефиса, то есть минимум — одна буква
    (`--a`): git принимает и её, а лишний отказ здесь дешевле пропуска.

    Форма `--флаг=значение` тоже узнаётся: сверяется часть до знака.
    """
    if not tok.startswith("--"):
        return False
    name = tok.split("=", 1)[0]
    return len(name) >= min_len and canonical.startswith(name)
